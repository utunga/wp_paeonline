...
<form id="wpfs-enter-security-code-form" class="wpfs-form wpfs-w-60" action="" method="post">
	...
	<div class="wpfs-form-lead">
		<div class="wpfs-form-description">
			<?php if ( MM_WPFS_Utils::isDemoMode() ) { ?>
				<?php esc_html_e( 'Enter any non-empty security code (DEMO MODE).', 'wp-full-stripe' ); ?>
			<?php } else { ?>
				<?php esc_html_e( 'Enter the security code sent to your email address.', 'wp-full-stripe' ); ?>
			<?php } ?>
		</div>
	</div>
	...
	<div class="wpfs-form-actions">
		...
		<a class="wpfs-btn wpfs-btn-link wpfs-nav-back-to-email-address"><?php esc_html_e( 'Back to e-mail address', 'wp-full-stripe' ); ?></a>
	</div>
</form>
